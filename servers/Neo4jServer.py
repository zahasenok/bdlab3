...
class Neo4jServer(object):
    def __init__(self):
        self.__driver = GraphDatabase.driver(
            config["NEO4J_URL"], auth=(config["NEO4J_LOGIN"], config["NEO4J_PASSWORD"])
        )
    ...
    def register(self, username, redis_id):
        with self.__driver.session() as session:
            session.run(
                "MERGE (u:user {name: $username, redis_id: $redis_id})"
                "ON CREATE SET u.online = false",
                username=username,
                redis_id=redis_id,
            )
    ...
    def login(self, redis_id):
        with self.__driver.session() as session:
            session.run(
                "MATCH (u:user {redis_id: $redis_id}) SET u.online = true",
                redis_id=redis_id,
            )
    ...
    def logout(self, redis_id):
        with self.__driver.session() as session:
            session.run(
                "MATCH (u:user {redis_id: $redis_id}) SET u.online = false",
                redis_id=redis_id,
            )
            ...
            try:
                ...
                for tag in message["tags"]:
                    session.write_transaction(
                        self.__add_tag_to_messages, messages_id, tag
                    )
    ...
    def __create_message_as_relation(tx, sender_id, consumer_id, message_id):
        result = tx.run(
            "MATCH(a: user {redis_id: $sender_id}), (b:user {redis_id: $consumer_id})"
    ...
    def __add_tag_to_messages(tx, messages_id, tag):
        tx.run(
            "MATCH ()-[r]-() where ID(r) = $messages_id "
            "FOREACH(x in CASE WHEN $tag in r.tags THEN [] ELSE [1] END | "
            "SET r.tags = coalesce(r.tags,[]) + $tag)",
    ...
    def deliver_message(self, redis_id):
        with self.__driver.session() as session:
            session.run(
                "MATCH (m:messages {redis_id: $redis_id }) SET m.delivered = true",
                redis_id=redis_id,
            )
    ...
    def mark_message_as_spam(self, redis_id):
        with self.__driver.session() as session:
            session.run(
                "MATCH (u1:user)-[r:messages]->(u2:user) "
                "WHERE $redis_id IN r.all AND NOT $redis_id IN r.spam "
                "SET r.spam = r.spam + $redis_id",
                redis_id=redis_id,
            )

    def get_users_with_tagged_messages(self, tags):
        result = self.__get_users_with_tagged_messages_from_db(tags)
        return self.__record_to_list(result, "name")

    def get_unrelated_users_with_tagged_messages(self, tags):
        list_of_names = self.__record_to_list(
            self.__get_users_with_tagged_messages_from_db(tags), "name"
        )
        unrelated_users = []
    ...
    def shortest_way_between_users(self, username1, username2):
        ...
        with self.__driver.session() as session:
            shortest_path = session.run(
                "MATCH p = shortestPath((u1:user)-[*..10]-(u2:user)) "
    ...
    def __pair_record_to_list(self, res, pull_out_value):
        ...
        for el in my_list:
            ...
            if list_el not in new_list and list_el[::-1] not in new_list:
                new_list.append(el)
        return [
            [el[0]._properties[pull_out_value], el[1]._properties[pull_out_value]]
            for el in new_list
        ]
